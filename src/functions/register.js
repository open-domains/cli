...
module.exports = async function register() {
    if(!account.has("username")) {
        ...
        console.log("To log in, run the command: `open-domains login`");
        return;
    }
...
`;
    ...
    fetch(
        `https://api.github.com/repos/open-domains/register/contents/domains/${subdomain}.${domain}.json`,
        {
            method: "GET",
            headers: {
                "User-Agent": username,
    ...
    const pr = await octokit.request("POST /repos/{owner}/{repo}/pulls", {
        ...
        body:  `Added \`${subdomain}.${domain}\` using the [CLI](https://cli.open-domains.net).`,
        head: username + ":main",
        base: "main"
    })
